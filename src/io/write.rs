...
pub fn write<Fd: AsFd + ?Sized>(fd: &Fd, buf: &[u8]) -> Result<usize, WriteError> {
    sys::call!(sys_write, fd.as_raw_fd(), buf.as_ptr(), buf.len()).io2::<WriteError>()
}
...
pub fn pwrite<Fd: AsFd + ?Sized>(fd: &Fd, buf: &[u8], offset: Off) -> Result<usize, WriteError> {
    sys::call_rd!(sys_pwrite64, fd.as_raw_fd(), buf.as_ptr(), buf.len(), offset).io2::<WriteError>()
}
...
pub fn writev<Fd: AsFd + ?Sized>(fd: &Fd, buf: &[IoVec<'_>]) -> Result<usize, WriteError> {
    sys::call_rd!(sys_writev, fd.as_raw_fd(), buf.as_ptr(), buf.len()).io2::<WriteError>()
}
