...
use crate::sys::SysErr;

#[inline]
pub(crate) unsafe fn call1_noret(nr: core::ffi::c_long, a1: i32) -> ! {
...
macro_rules! defv2 {
    ($($no:literal $abi:tt $name:ident $entryp:ident)*) => {$(
        #[doc = concat!(" `", stringify!($name), "`")]
        pub struct $entryp(());
        impl $entryp {
            pub(crate) const NO: core::ffi::c_long = $no;
        }
        impl SysErr for $entryp {
...
macro_rules! call {
    ($nr:ident
     ...
     $(, $a6:expr
        ...
        core::arch::asm!(
            "syscall",
            inlateout("rax") crate::sys::arch::$nr::NO => ret,
            $(in("rdi") $a1,
            $(in("rsi") $a2,
            $(in("rdx") $a3,
            $(in("r10") $a4,
        ...
        core::mem::transmute::<
            core::ffi::c_long,
            crate::sys::SysResRaw<_, crate::sys::arch::$nr>
        >(ret)
    } };
}
